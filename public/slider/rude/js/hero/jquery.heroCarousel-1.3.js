    ...
    return this.each(function () {
        ...
        if (elements.length > 2) {
            ...
            elements
                ...
                .before(elements.filter(":last"));

            var carouselWidth = Math.round(
                    childWidth * carousel.children().length
                ),
                carouselMarginLeft =
                    "-" +
                    Math.round(childWidth + Math.round(childWidth / 2)) +
                    "px";
            ...
            carousel.before(
                '<ul class="hero-carousel-nav"><li class="prev"><a href="#">' +
                    options.prevText +
                    '</a></li><li class="next"><a href="#">' +
                    options.nextText +
                    "</a></li></ul>"
            );
                    ...
                    if (carouselNav.attr("data-disabled") !== "true") {
                        ...
                        if (navItem.data("disabled") === false) {
                            options.onStart(
                                carousel,
                                carouselNav,
                                elements.eq(currentItem),
                                options
                            );
                            if (isPrevious) {
                                animateItem(
                                    elements.filter(":last"),
                                    "previous"
                                );
                            } else {
            ...
            function animateItem(object, direction) {
                ...
                var carouselPosLeft = parseFloat(carousel.position().left),
                    carouselPrevMarginLeft = parseFloat(
                ...
                if (direction === "previous") {
                    ...
                    var marginLeft = Math.round(
                        carouselPrevMarginLeft - childWidth
                    );
                    var plusOrMinus = "+=";
                    //custom
            ...
            function finishCarousel() {
                ...
                options.onComplete(
                    carousel,
                    carousel.prev(".hero-carousel-nav"),
                    elements.eq(currentItem),
                    options
                );
            }
            ...
            options.onLoad(
                carousel,
                carouselNav,
                carousel.children().eq(currentItem),
                options
            );
        }
    });
