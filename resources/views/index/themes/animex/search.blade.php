        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class=" product__pagination">
                                            <a href="{{$path}}&p={{$currentPage - 1}}" {{ ($currentPage<=1) ? 'hidden'
                                                : '' }}><i class="fa fa-angle-double-left"></i></a>
                                            @for ($i = 1; $i <= $pageCount; $i++) @if ($currentPage==$i) <a
                                                href="{{$path}}&p={{$i}}" class="current-page">{{ $i }}</a>
                                        @else
                                        <a href="{{$path}}&p={{$i}}" class="current-page">{{ $i }}</a>
                                        @endif
                                        @endfor
                                        <a href="{{$path}}&p={{$currentPage + 1}}" {{ ( $currentPage + 1>$pageCount)
                                            ? 'hidden' : '' }}><i class="fa fa-angle-double-right"></i></a>
                                </div>
                            </div>
                        </div>
                    </div>
