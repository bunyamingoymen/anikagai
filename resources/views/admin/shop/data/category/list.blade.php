    ...
    @if ($list)
                        ...
                        <div class="col-lg-12" style="display: inline-block;">
                            @if ($create == 1)
                                <a class="btn btn-primary mb-3" style="float: right;"
                                    href="{{ route('admin_shop_category_create') }}">+ Yeni</a>
                            @endif
                        </div>
        ...
        <script>
            function changePage(page) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route('admin_shop_category_get_data') }}',
                    data: pageData,
                ...
                function deleteValue(code, name) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            var html =
                                `<form action='{{ route('admin_shop_category_delete') }}' method="POST" id="deleteForm"> @csrf`;
                        ...
                        cellRenderer: function(params) {
                            ...
                            @if ($update == 1)
                                html += `<div class="mr-2 ml-2">
                                        <a class="btn btn-warning btn-sm" href="{{ route('admin_shop_category_update') }}?code=${params.data.code}" data-toggle="tooltip" data-placement="right" title="Güncelle"><i class="fas fa-edit"></i></a>
                                    </div>`
                            @endif
                            @if ($delete == 1)
                                html += `<div class="mr-2 ml-2">
