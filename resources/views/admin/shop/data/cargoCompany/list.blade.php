                        ...
                        <div class="col-lg-12" style="display: inline-block;">
                            @if ($create == 1)
                                <a class="btn btn-primary mb-3" style="float: right;"
                                    href="{{ route('admin_shop_cargo_company_create') }}">+ Yeni</a>
                            @endif
                        </div>
                    ...
                    success: function(response) {
                        ...
                        for (let i = 0; i < items.length; i++) {
                            var rowItem = {
                                ...
                                name: sendData(items[i].value), // kargo ismi
                                image: sendData(items[i].optional), // kargo fotoğrafı
                                description: sendData(items[i].optional_2), // kargo açıklaması
                            }
                ...
                function deleteValue(code, name) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            var html =
                                `<form action='{{ route('admin_shop_cargo_company_delete') }}' method="POST" id="deleteForm"> @csrf`;
                        ...
                        cellRenderer: function(params) {
                            ...
                            @if ($update == 1)
                                html += `<div class="mr-2 ml-2">
                                        <a class="btn btn-warning btn-sm" href="{{ route('admin_shop_cargo_company_update') }}?code=${params.data.code}" data-toggle="tooltip" data-placement="right" title="Güncelle"><i class="fas fa-edit"></i></a>
                                    </div>`
                            @endif
                            @if ($delete == 1)
                                html += `<div class="mr-2 ml-2">
