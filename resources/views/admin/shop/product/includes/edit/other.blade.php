    ...
    <select name="cargo_company" id="cargo_company" style="width: 100%;">
        @if (isset($cargo_company))
            <option value="{{ $cargo_company->code }}" selected>
                <span> <img class="rounded-circle header-profile-user" src="{{ url($cargo_company->optional) }}"
                        alt="cargo_image" style="width:30px; height:30px; margin-right: 10px;" />
                    {{ $cargo_company->value }} </span>
            </option>
        @endif
    </select>
</div>
...
<script>
    function configCargoCompany() {
        ...
        function formatResult(item) {
            ...
            if (item.image) {
                imageMarkup =
                    `<img class="rounded-circle header-profile-user" src="../../../../${item.image}" alt="cargo_image" style="width:30px; height:30px; margin-right: 10px;" />`;
            }
