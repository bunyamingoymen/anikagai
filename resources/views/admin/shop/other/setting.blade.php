                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{route('admin_shop_archive_and_delete_settings')}}" method="POST">
                            @csrf
                            <div class="custom-control custom-checkbox mb-2">
                                <input type="checkbox" class="custom-control-input" id="add_archive" name="add_archive" {{isset($addArchive) && $addArchive->value == 1 ? 'checked' : ''}} onchange="changeAddArchive()">
                                <label class="custom-control-label" for="add_archive">Ürün satılmaz ise belli bir süre sonra arşiv'e eklensin</label>
                            </div>
                            <div id="archiveTimeDiv" class="col-md-2 mb-3" style="{{isset($addArchive) && $addArchive->value == 1 ? '' : 'display:none'}}">
                                ...
                                <input type="number" class="form-control" name="archive_time" id="archive_time" placeholder="Gün sayısı giriniz" value="{{$archiveTime ?? ''}}">
                            </div>

                            <div class="custom-control custom-checkbox mb-2">
                                <input type="checkbox" class="custom-control-input" id="delete_automatic" name="delete_automatic" {{isset($deleteAutomatic) && $deleteAutomatic->value == 1 ? 'checked' : ''}} onchange="changeDeleteAutomatic()">
                                <label class="custom-control-label" for="delete_automatic">Ürün satılmaz ise belli bir süre sonra silinsin</label>
                            </div>
                            <div id="deleteAutomaticDiv" class="col-md-2 mb-3" style="{{isset($deleteAutomatic) && $deleteAutomatic->value == 1 ? '' : 'display:none'}}">
                                ...
                                <input type="number" class="form-control" name="delete_date" id="delete_date" placeholder="Gün sayısı giriniz" value="{{$deleteTime??''}}">
                            </div>
                            <div class="mt-3">
                                <button type="submit" class="btn btn-info"><i class="fas fa-save"></i> Kaydet</button>
                            </div>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="">
                            <div class="custom-control custom-checkbox mb-2">
                                <input type="checkbox" class="custom-control-input" id="same_theme" checked="">
                                <label class="custom-control-label" for="same_theme">Ana site ile aynı logoya sahip olsun</label>
                            </div>
                            <div class="mt-3">
                                <button type="submit" class="btn btn-info"><i class="fas fa-save"></i> Kaydet</button>
                            </div>
